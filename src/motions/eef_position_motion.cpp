...
EEFPositionMotion::EEFPositionMotion(size_t eef_index,
                                     ConstRefVector3b selected,
                                     const pinocchio::SE3 &origin_pose,
                                     double Kp, double Kd)
    : MotionWithTrajectory(selected, Kp, Kd), eef_index_(eef_index),
      origin_(origin_pose) {}

Eigen::VectorXd EEFPositionMotion::GetPositionError(
    const RefVectorXd pos, const pinocchio::Model &model, pinocchio::Data &data,
    ConstRefVectorXd q, ConstRefVectorXd qv) const {
  return pos - origin_.actInv(data.oMf[eef_index_].translation())(indexes_);
}

Eigen::VectorXd EEFPositionMotion::GetVelocityError(
...
Eigen::VectorXd
EEFPositionMotion::GetAcceleration(const pinocchio::Model &model,
