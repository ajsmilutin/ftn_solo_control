...
FixedPointsEstimator::FixedPointsEstimator(double dt,
  ...
  double cutoff = 200;
  qv_filter_.setup(6, sample_rate, cutoff);
}
...
void FixedPointsEstimator::Init(double t, ConstRefVectorXd q,
  ...
  std::cout<<"TTTTTTTTTTTTTTTTTTTTTTTTT INIT"<<std::endl;
...
void FixedRobotEstimator::Estimate(double t, ConstRefVectorXd q,
  ...
  Eigen::VectorXd qv_copy = qv;
  estimated_qv_.tail(num_joints_) = qv_filter_.filter(qv_copy);
  // alpha * estimated_qv_.tail(num_joints_) + (1 - alpha) * qv;
